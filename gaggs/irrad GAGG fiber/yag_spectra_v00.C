...
void draw2(TPad* lPad, TGraphErrors* tg1, TGraphErrors* tg2){
  lPad->SetFillColor(0);
  lPad->Clear();
...
void yag_spectra_v00(Double_t c1=1, Double_t c2=1, Double_t xwl=550, Double_t dxwl=2.5, Double_t xf1=0, Double_t xf2=10)
{
  Double_t koef[2]; koef[0]=c1; koef[1]=c2;

  Double_t wlmin=400, wlmax=700;
  Int_t npwl=28;
  ...
  if(cr)cr->Close();
  cr=new TCanvas("cr","att len",900,600);
  cr->cd();
  cr->SetFillColor(0);
  ...
  char funcname[256];
  sprintf(funcname,"func_%d",(int)xwl);
  TF1* tfr = new TF1(funcname,"([0]*exp(-x*2./[1]))",xf1,xf2);
  Int_t np=tgr->GetN();
  ...
  TFitResultPtr r=tgr->Fit(funcname,"SR");
